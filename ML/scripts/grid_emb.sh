...
# Train/Test: True
#-----------------------------------------------------------------------------
for classifier in naive logistic;do
  for technique in d2v fasttextnw fasttextsw glove google;do
    for dataset in arxivp;do 
      #save path into variable
      path=grid_wiki_arxiv/$classifier/$technique-$dataset-mean-$classifier.txt
      #create folder structure
      mkdir -p $(dirname $path) && touch $path
      #call python code
      python3 text_mining/main.py \
        --train datasets_vector_train_p/$technique-machinep-mean.arff \
        --test datasets_vector_arxiv/$technique-${dataset}-mean.arff \
        --classifier $classifier \
        --grid True \
        --cpu 16 \
        > $path
